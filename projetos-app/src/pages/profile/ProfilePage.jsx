          ...
          <div className="max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {activeSection === 'perfil' && (
              <div className="space-y-10">
                {/* Avatar hero — visível sem precisar entrar na aba Identidade 3D */}
                <div className="flex items-center gap-8 p-6 bg-[var(--surface-1)] border border-[var(--border-subtle)] rounded-2xl">
                  <div className="shrink-0 w-[160px] h-[200px] rounded-2xl overflow-hidden border-2 shadow-lg transition-all"
                    style={{ borderColor: `${ac}44`, boxShadow: `0 8px 24px ${ac}22` }}>
                    <AvatarWidget userId={user?.id} width={160} height={200} />
                  </div>
                  <div className="flex flex-col gap-3 min-w-0">
                    <div>
                      <p className="text-xl font-semibold tracking-tight">{form.display_name || user?.email?.split('@')[0] || 'Usuário'}</p>
                      <p className="text-xs text-muted-foreground/50 mt-0.5">{user?.email}</p>
                      {form.role && <p className="text-xs text-muted-foreground mt-1">{form.role}</p>}
                    </div>
                    <button
                      onClick={() => setActiveSection('identidade')}
                      className="self-start flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold bg-[var(--surface-2)] border border-[var(--border-subtle)] hover:bg-[var(--surface-3)] transition-all"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      Randomizar Identidade
                    </button>
                  </div>
                </div>
