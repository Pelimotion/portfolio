...
function showPortfolio() {
  autoSave(); currentSection = 'portfolio'; currentKey = null; highlightSb(null);
  const P = D.portfolio || {};
  document.getElementById('main-content').innerHTML = `
...
function openEditor(type, key) {
  autoSave(); currentSection = type; currentKey = key;
  highlightSb(key);
...
function addExp() {
  saveCurriculumToMem(); const C = D.curriculum; C.experience = C.experience || [];
...
function addEdu() {
  saveCurriculumToMem(); const C = D.curriculum; C.education = C.education || [];
...
function addLang() {
  saveCurriculumToMem(); const C = D.curriculum; C.languages = C.languages || [];
...
function addSkillCat() {
  saveCurriculumToMem(); const C = D.curriculum; C.skills = C.skills || {};
  C.skills[`New Category ${Object.keys(C.skills).length + 1}`] = []; showCurriculum(); markUnsaved();
}
function removeSkillCat(i) {
  saveCurriculumToMem(); const C = D.curriculum;
...
function showCoverLetter() {
  autoSave(); currentSection = 'coverLetter'; currentKey = null; highlightSb(null);
  ...
  document.getElementById('main-content').innerHTML = `
    ...
    <div class="card"><div class="card-title"><span class="icon">✍️</span> Letter Content</div>
      ...
      <div class="field-pair">
      <div class="field-pair">
        <div class="field"><label>Hero Subtitle — <span style="color:var(--fg3)">EN</span></label><input id="f-clSub" value="${esc(CL.subtitle || '')}" oninput="markUnsaved()"><button type="button" class="btn" style="margin-top:6px;font-size:10px;padding:4px 10px" onclick="autoTranslate('f-clSub','f-clSub_pt','en','pt')">↕ Auto-Traduzir</button></div>
        <div class="field"><label>Hero Subtitle — <span style="color:#34d399">PT</span></label><input id="f-clSub_pt" value="${esc(CL.subtitle_pt || '')}" oninput="markUnsaved()"></div>
      </div>
...
function saveCoverLetterToMem() {
  ...
  const v = id => { const el = document.getElementById(id); return el ? el.value.trim() : ''; };
  CL.subtitle = v('f-clSub'); CL.subtitle_pt = v('f-clSub_pt');
...
function showApplications() {
  autoSave(); currentSection = 'applications'; currentKey = null; highlightSb(null);
